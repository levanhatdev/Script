...
if game:GetService("CoreGui"):FindFirstChild("Tommy Teo Hub") then
	game:GetService("CoreGui"):FindFirstChild("Tommy Teo Hub"):Destroy()
end
local DiscordLib = {}
...
function DiscordLib:Window(text,mainclr,cls)
	...
	function ServerHold:Server(text, img)
		...
		spawn(function()
			while wait() do
				TimeGlobal.Text = "Giờ | "..os.date("%H")..":"..os.date("%M")..":"..os.date("%S")
			end
		end)
...
local win = DiscordLib:Window("Blox Fruits - V1.2", _G.Color, _G.Toggle)

local serv = win:Server("Tommy Teo Hub","")

local AutoFarm = serv:Channel("Trang Chủ", "http://www.roblox.com/asset/?id=7040391851")

Time = AutoFarm:Label("Thời Gian Tham Gia Sever")
...
function UpdateTime()
    ...
    Time:Refresh("Giờ : "..Hour.." Phút : "..Minute.." Giây : "..Second)
end
...
AutoFarm:Toggle("Tự Động Cày Level", _G.AutoFarmLevel, function(vu)
    _G.AutoFarm = vu
	if _G.AutoFarm and SelectToolWeapon == "" then
		DiscordLib:Notification("Thông Báo!","Vui Lòng Chọn Vũ Khí","Okay")
	else
		Auto_Farm = vu
		SelectMonster = ""
		if vu == false then
...
AutoFarm:Toggle("Tự Động Đặt Điểm Hồi Sinh", false, function(vu)
    AutoSetSpawn = vu
end)

if New_World then
    AutoFarm:Toggle("Tự Động Nhà máy", false, function(vu)
        Factory = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
if Three_World then
	AutoFarm:Toggle("Tự Động Săn", _G.AutoElite, function(vu)
		EliteHunter = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
	...
	AutoFarm:Toggle("Tự Động Lấy Enma/Yama", _G.AutoYama, function(vu)
		AutoYama = vu
	end)
	AutoFarm:Toggle("Tự Động Lấy Rainbow Haki", false, function(vu)
		AutoRainbow = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
if Old_World then
	AutoFarm:Toggle("Tự Động Sang Sea Mới", _G.AutoNewworld, function(vu)
		Auto_Newworld = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
AutoFarm:Toggle("Tự Động Superhuman", _G.AutoSuperhuman, function(vu)
    Superhuman = vu
end)

AutoFarm:Toggle("Tự Động Full Superhuman", _G.FullyAutoSuperhuman, function(vu)
    AutoFullySuperhuman = vu
end)

AutoFarm:Toggle("Tự Động Electric Claw", false, function(vu)
    AutoElectricClaw = vu
...
AutoFarm:Toggle("Tự Động Lấy Phụ kiện", _G.AutoAccessory, function(Value)
...
AutoFarm:Button("Làm Mới Vũ Khí", function()
    SelectWeapon:Clear()
    for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
        if v:IsA("Tool") then
...
if New_World then
    AutoFarm:Toggle("Tự Động Mua Legendary Sword", _G.Auto_Legendary_Sword, function(Value)
        LegebdarySword = Value    
    end)
end
if not Old_World then
    AutoFarm:Toggle("Tự Động Mua Enchancement >= 1500 F", _G.AutoEnchancement, function(Value)
        Enchancement = Value    
    end)
end
...
AutoFarm:Toggle("Tự Động Cày Thông Khạo Trái Ác Quỷ", false, function(vu)
...
AutoFarm:Toggle("Tự Động Cày Thông Khạo Súng", false, function(vu)
...
AutoFarm:Toggle("Tự Động Cày Thông Khạo Magnet", false, function(vu)
	MasteryMagnet = vu
end)

local SelectWeapon = AutoFarm:Dropdown("Vui Lòng Chọn Vũ Khí",Wapon,function(Value)
    WeaponMastery = Value
end)
...
AutoFarm:Toggle("Tự Động Giết Boss", false, function(vu)
	AutoFarmBoss = vu
...
AutoFarm:Toggle("Tự Động Giết Boss Có Nhiệm Vụ", false, function(vu)
	BossQuest = vu
	if BossQuest then
		DiscordLib:Notification("Thông Báo!","Tự Động Giết Boss Có Nhiệm Vụ Đang Lỗi:<!","Okay")
	end
end)

local BossName = AutoFarm:Dropdown("Chọn Boss", Boss, function(Value)
    SelectBoss = Value
end)
...
AutoFarm:Button("Làm Mới Boss", function()
    BossName:Clear()
    for i,v in pairs(game.ReplicatedStorage:GetChildren()) do
        if string.find(v.Name, "Boss") then
            if v.Name == "Ice Admiral [Lv. 700] [Boss]" then
...
AutoFarm:Toggle("Tự Động Chọn Quái", false, function(vu)
...
if New_World then
	AutoFarm:Toggle("Tự Động Nhận Nhiệm Vụ Bartilo", false, function(vu)
		CheckBarto = vu
		...
		if Success and CheckBarto == true then
			DiscordLib:Notification("Thông Báo!","Thành Công","Ok")
	...
	AutoFarm:Toggle("Tự Động Evo Race", false, function(vu)
		CheckEvo = vu
		if not game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 and CheckEvo then
			DiscordLib:Notification("Thông Báo!","Bạn phải thành công nhiệm vụ Bartilo","Okay")
	...
	AutoFarm:Toggle("Tự Động Rengoku", false, function(vu)
		AutoRengoku = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
	...
	AutoFarm:Toggle("Tự Động Ectoplasm", false, function(vu)
		AutoEcto = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
if Three_World then
	AutoFarm:Toggle("Tự Động Citizen Có Nhiệm Vụ", false, function(vu)
		CheckCitizen = vu
		TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
AutoFarm:Label(""):Refresh("Sự Kiện Giáng Sinh (Hết Hạn)")

local Bone = AutoFarm:Label("")

spawn(function()
	while game:GetService("RunService").Heartbeat:wait() do
		Bone:Refresh("Kẹo : "..tostring(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies","Check")))
	end
end)

AutoFarm:Button("Mua Thuyền", function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies","Buy",1,3)
end)

AutoFarm:Button("Mua Reset Chỉ Số", function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies","Buy",1,2)
end)

AutoFarm:Button("Mua Random", function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies","Buy",1,1)
end)

AutoFarm:Button("Tự Động Mua Elf Hat", false, function(vu)
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies", "Buy", 3, 1)
end)

AutoFarm:Button("Tự Động Mua Santa Hat", false, function(vu)
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies", "Buy", 3, 1)
end)

AutoFarm:Button("Tự Động Mua Sleigh", false, function(vu)
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Candies", "Buy", 3, 3)
end)

AutoFarm:Toggle("Tự Động Random", false, function(vu)
	AutoBuySurprise = vu
end)

AutoFarm:Toggle("Tự Động Mua EXP x2 [ Kẹo ]", false, function(vu)
    AutoBuyEXPBone = vu
end)
...
if Three_World then
    ...
    AutoFarm:Toggle("Tự Động Thử Vận May [Bia Mộ]", false, function(vu)
        TryLuck = vu
    end)

    AutoFarm:Toggle("Tự Đông Cầu Nguyện [Bia Mộ]", false, function(vu)
        Pray = vu
    end)

    AutoFarm:Toggle("Tự Động Cày Kẹo", false, function(vu)
		Auto_Bone = vu
		if vu == false then
			wait(1)
			TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
    ...
    local SelectWeaponEvent = AutoFarm:Dropdown("Vui Lòng Chọn Vũ Khí",Wapon,function(value)
        EventWeapon = value
    end)
...
AutoFarm:Toggle("Tự Động Cày Haki Quan Sát", AutoFarmKen, function(vu)
...
spawn(function()
	while game:GetService("RunService").Heartbeat:wait() do
		...
		ObservationLevel:Refresh("Cấp Độ Haki Quan Sát : "..tostring(Value))
	end
end)

local Stats = serv:Channel("Stats","http://www.roblox.com/asset/?id=7040410130")

Stats:Toggle("Nắm Đấm", _G.Melee, function(vu)
    Mad = vu
end)
...
Stats:Label(""):Refresh("Thống Kê Người Chơi")
...
spawn(function()
	while game:GetService("RunService").Heartbeat:wait() do
		...
		Point:Refresh("Cấp Độ : "..tostring(game:GetService("Players").LocalPlayer.Data.Level.Value))
...
local Tp = serv:Channel("Dịch Chuyện","http://www.roblox.com/asset/?id=7044226690")
...
Tp:Toggle("Ctrl + Nhấn Để Dịch Chuyển", false, function(vu)
	CTRL = vu
end)

Tp:Button("Dừng", function()
    Clip = false
    TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
end)
...
elseif New_World then
    ...
    Tp:Button("Cánh Đồng Hoa Hướng Dương", function()
        TP2(CFrame.new(-1576.7171630859, 198.61849975586, 13.725157737732))
    end)

    Tp:Button("Núi Jeramy", function()
...
Players:Button("Làm Mới Người Chơi", function()
    PlayerName = {}
    SelectPlayer:Clear()
...
Players:Toggle("Nhìn Chế Độ Người Chơi", false, function(vu)
    Spectate = vu
...
Players:Toggle("Giết Tất Cả Người Chơi + Hop [Phòng Thủ 1 Chỉ]", _G.AutoBounty, function()
	_G.AutoBounty = vu
end)
...
local SelectWeapon = Players:Dropdown("Vui Lòng Chọn Vũ Khí",Wapon,function(Value)
    SelectKillWeapon = Value
end)

Players:Button("Làm Mới Vũ Khí", function()
    SelectWeapon:Clear()
	for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
		if v:IsA("Tool") then
			SelectWeapon:Add(v.Name)
...
Players:Toggle("Ghim Súng", false, function(vu)
	Aimbot = vu
end)

Players:Toggle("Ghim Skill Gần Nhất", false, function(vu)
	AimSkillNearest = vu
end)
...
Players:Slider("Chạy Nhanh", 0,1000,16, function(t)
...
Players:Slider("Nhảy Cao", 0,1000,50, function(t)
    game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = t
end)

Players:Slider("Không Trọng Lực", 0,500,196, function(t)
    workspace.Gravity = t
end)

local Raid = serv:Channel("Định Vị - Raid","http://www.roblox.com/asset/?id=7251993295")
if New_World then
	Raid:Button("Dịch Chuyển Tới Chỗ Mua Chip", function()
		TP2(CFrame.new(-6438.73535, 250.645355, -4501.50684))
	end)
elseif Three_World then
	Raid:Button("Dịch Chuyển Tới Chỗ Raid", function()
		TP2(CFrame.new(-5057.146484375, 314.54132080078, -2934.7995605469))
	end)
end

Raid:Toggle("Tự Động Giết", false, function(vu)
    Killaura = vu
end)

Raid:Toggle("Tự Động Thức Tỉnh", false, function(vu)
    AutoAwakener = vu
end)

Raid:Toggle("Tự Động Sang Đảo Mới", false, function(vu)
    NextIsland = vu
end)

Raid:Line()

Raid:Toggle("Tự Động Raid", false, function(vu)
    _G.AutoRaid = vu
end)

Raid:Dropdown("Chọn Chip Raid", {"Flame","Ice","Quake","Light","Dark","String","Rumble","Magma","Human: Buddha","Sand"}, function(vu)
    _G.SelectRaid = vu
end)

Raid:Toggle("Tự Động Mua Microchip", false, function(vu)
    AutoBuychip = vu
end)
				...
				if game:GetService("Players")["LocalPlayer"].PlayerGui.Main.Timer.Visible == false then
					...
					if game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1") then
						game:GetService("StarterGui"):SetCore("SendNotification",
							{
								Title = "Tự Động Raid",
								Text = "Có một số người trong Raid",
								Icon = "",
								Duration = 4
							}
						)
...
Raid:Toggle("Tự Động Đến Raid", false, function(vu)
	AutoLowRaid = vu
end)

Raid:Toggle("Mua Chip Chip Đến Raid", false, function(vu)
	AutoBuyChiplawraid = vu
end)

local SelectWeaponLaw = Raid:Dropdown("Vui Lòng Chọn Vũ Khí",Wapon,function(vu)
	SelectToolWeaponLaw = vu
end)

Raid:Button("Làm Mới Vũ Khí", function()
...
Raid:Toggle("Định Vị Trái Ác Quỷ",false,function(Value)
    DevilESP = Value
...
SelectDevil = ""
CheckF = false
Devil:Dropdown("Tự Động Mua Trái Ác Quỷ",
{
    "Bomb-Bomb",
    "Spike-Spike",
    "Chop-Chop",
...
Devil:Toggle("Mua Trái Ác Quỷ Xạ Thủ",false,function(vu)
    BuyFruitSinper = vu
end)
spawn(function()
    while wait(.1) do
...
Devil:Toggle("Tự Động Cửa Hàng Trái Ác Quỷ", false, function(vu)
	AutoStoreFruit = vu
end)
...
Devil:Toggle("Mang Trái Ác Quỷ", _G.BringFruit, function(value)
	BringFruit = value
end)
...
Devil:Toggle("Tự Động Vứt Trái Ác Quỷ", false, function(vu)
    Drop = vu
end)
...
Devil:Button("Tự Động Random Trái Ác Quỷ", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Cousin","Buy")
end)

Devil:Toggle("Tự Động Mua Random Trái Ác Quỷ", false, function(v)
    DevilAutoBuy = v
end)
...
local BuyItem = serv:Channel("Mua Vật Phẩm","http://www.roblox.com/asset/?id=7294901968")

BuyItem:Label("Khả Năng")
BuyItem:Button("Nhảy Cao", function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyHaki","Geppo")
end)
BuyItem:Button("Haki Quan Sát", function()
...
BuyItem:Toggle("Mua Khả Năng Khả Năng", false, function(t)
    Abilities = t
...
Setting:Button("Sao Chép Link Discord : Tommy Teo Hub", function()
	setclipboard("https://discord.gg/c5g4whs4uZ")
end)

Setting:Line()

Setting:Toggle("Nhấn Nút Home Dừng Tween", true, function(vu)
...
Setting:Toggle("Đánh Nhanh", _G.FastAttack, function(vu)
    FastAttack = vu
end)

Setting:Toggle("Tự Động Haki", true, function(vu)
    Auto_Haki = vu
end)

Setting:Toggle("Tự Động Haki Quan Sát", false, function(vu)
	AutoObservation = vu
end)

Setting:Label("Cày Đặt Tự Động Cày :)")
...
Setting:Slider("Khóa Cấp Độ", 1,2200,_G.LevelMax, function(value)
	LevelLock = value
end)

Setting:Toggle("Cấp Độ Khóa Kick", false, function(vu)
	LevelLockKick = vu
end)

Setting:Toggle("Khóa Cấp Độ Đóng Trò Chơi", _G.LevelLock, function(vu)
	LevelLockClose = vu
end)
...
Setting:Toggle("Tàn Hình Mob", true, function(vu)
    HiddenMon = vu
end)

Setting:Line()

Setting:Label("Cày Đặt Tự Động Cày Thông Khạo")
...
Misc:Button("Hợp Máy Chủ Hạ", function()
	HopLowerServer()
end)

Misc:Button("Chuyển Sever", function()
	DiscordLib:Notification("Thông Báo!", "Vui Lòng Đợi Chuyển Sever", "Ok")
	Teleport()
end)

Misc:Button("Vô Lại Server", function()
	game:GetService("TeleportService"):Teleport(game.PlaceId, game:GetService("Players").localPlayer)
end)

Misc:Line()

Misc:Button("Vào Team Hải Tặc", function()
    local args = {
        [1] = "SetTeam",
        [2] = "Pirates"
    }
...
Misc:Button("Vào Team Hải Quân",function()
    local args = {
        [1] = "SetTeam",
        [2] = "Marines"
    }
...
Misc:Button("Cửa Hàng Trái Ác Quỷ", function()
    local args = {
        [1] = "GetFruits"
    }
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
...
Misc:Button("Hàng Tồn Kho", function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")
    game.Players.localPlayer.PlayerGui.Main.Inventory.Visible = true
end)

Misc:Button("Hàng Tồn Kho Trái Ác Quỷ", function()
	local args = {
...
if New_World then
	Misc:Button("Dịch Chuyển Đến Hoa", function()
...
Misc:Toggle("Không Có Thời Gian Hồi Chiêu", false, function(Value)
    nododgecool = Value
    NoDodgeCool()
end)

Misc:Toggle("Năng Lượng Inf",false,function(value)
    InfinitsEnergy = value
    originalstam = LocalPlayer.Character.Energy.Value
end)

Misc:Toggle("Khả Năng inf", false, function(vu)
	InfAbility = vu
end)
...
Misc:Toggle("Tự Động Nhấp", false, function(value)
    AutoClick = value 
end)

Misc:Toggle("Đi Trên Nước", false, function(vu)
    _G.Water = vu
end)
...
if Three_World then
	Misc:Button("Tự Động Lấy Đuốc", function()
		if not game:GetService("Players").LocalPlayer.Character:FindFirstChild("Holy Torch") and not game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Holy Torch") then
			DiscordLib:Notification("Thông Báo", "Bạn không có ngọn đuốc thần thánh", "Okay")
		else
			HolyTorch = true
...
Misc:Button("Chế Độ Đồ Đẹp", function()
	BeautifulMode()
end)

Misc:Toggle("Cơ Thể Phát Sáng", false, function(vu)
	_G.LightMode = vu
end)
...
Misc:Toggle("Xuyên Mọi Vật [Pha Ke :)]", false, function(vu)
    noclip = vu
end)

Misc:Button("Loại Bỏ Mức Thấp", function()
...
Misc:Button("Đổi Tất Cả Mã", function()
...
DiscordLib:Notification("Cảnh Báo!", "Đây Là Bản Beta Nên Toàn Nhiều Lỗi!", "Okay")
